  ...
  methods: {
    ...
    handleDownMerge() {
      ...
      this.rightMenuSelectValue.trs[this.trIndex].tds.forEach((element, index) => {
        ...
        if (
          element.rowspan > this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex].rowspan
        ) {
          rows += 1;
        }
      });
    ...
    handleRightMerge() {
      ...
      this.rightMenuSelectValue.trs[this.trIndex].tds = this.rightMenuSelectValue.trs[
        this.trIndex
      ].tds.filter((item, index) => {
  ...
  render() {
    return (
      <div
        class="form-panel"
        onClick={() => {
          this.$emit('handleSetSelectItem', { key: '' });
        }}
      >
        {this.data.list.length === 0 ? <p class="hint-text">从左侧选择控件添加</p> : null}
        <a-form
          class="a-form-box k-form-build"
          form={this.form}
          layout={this.data.config.layout}
          hide-required-mark={this.data.config.hideRequiredMark}
          style={this.data.config.customStyle}
        >
          <draggable
            ...
            animation={180}
            handle=".drag-move"
            onAdd={this.deepClone}
            onStart={event => {
              console.log('ddd');
              this.dragStart(event, this.data.list);
            }}
          >
            <transition-group tag="div" name="list" class="list-main">
              {this.data.list.map(record => {
                return (
                  <layoutItem
                    key={record.key}
                    record={record}
        ...
        {this.showRightMenu ? (
          <div style={{ top: this.menuTop + 'px', left: this.menuLeft + 'px' }} class="right-menu">
            <ul>
              <li onClick={this.handleDownMerge}>
                <a-icon type="caret-down" />
                向下合并
              </li>
              <li onClick={this.handleRightMerge}>
                <a-icon type="caret-right" />
                向右合并
              </li>
              <li onClick={this.handleAddCol}>
                <a-icon type="border-horizontal" />
                增加一列
              </li>
              <li onClick={this.handleAddRow}>
